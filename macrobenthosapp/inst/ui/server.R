shinyServer(function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(pts2, options=list(center = c(51.18, 4.3), zoom = 10)) %>%
      ...
      addCircleMarkers(group="densiteit", radius =~pts2$varsize, color="white", weight = 1,
                       layerId=~locatie,
                       fillColor=~pal2(pts2var), opacity = .6,
                       fillOpacity = .6,
                       label = ~htmlEscape(locatie)) %>%
      addLegend("bottomright", pal = pal2,
                title=title,
                values = ~pts2var, opacity = 1) %>%
      addLayersControl(baseGroups = c("CartoDB.Positron","Google Hybrid","Google Aerial","Google Roads",
                                      "Esri satellite","OpenStreetMap","OpenTopoMap"),
    ...
    isolate({
      ...
      content <-  paste(
        ...
        paste0("<b>",as.character(sgh$soort),"</b><br>",
               "   ",as.character(round(sgh$densiteit, 1))," ind/m<sup>2</sup> - - - ",
  ...
  output$grafiek1 <- renderPlot({
    ...
    if (input$gt1=="cnt1") {
      if (input$jaar!="alle jaren") {
        ...
        grafdat <- dcast(grafdat, jaar + waterloop + fysiotoop ~ ., length, drop = FALSE)
        ggplot(grafdat, aes(x=waterloop,y=., fill=fysiotoop)) +
          geom_bar(stat="identity", position=position_dodge()) + scale_x_discrete(drop=FALSE) +
        ...
        grafdat <- dcast(grafdat, waterloop + fysiotoop ~ ., length, drop = FALSE)
        ggplot(grafdat, aes(x=waterloop,y=., fill=fysiotoop)) +
          geom_bar(stat="identity", position=position_dodge()) + scale_x_discrete(drop=FALSE) +
          scale_y_continuous(breaks = function(x) unique(floor(pretty(seq(0, (max(x) + 1) * 1.1))))) +
          labs(x="",y="aantal stalen")
      ...
      grafdat <- dcast(grafdat, jaar + waterloop + fysiotoop + locatie ~ soort, sum,
                       value.var = input$variabele)
      grafdat <- grafdat[c("jaar","waterloop","fysiotoop","locatie",input$soort)]
      names(grafdat)[names(grafdat)==names(grafdat)[5]] <- input$variabele
      if (input$variabele=="densiteit")
      {
        ggplot(grafdat, aes(x=waterloop,y=densiteit+min(grafdat$densiteit[grafdat$densiteit!=0]), fill=fysiotoop)) +
          geom_boxplot() + scale_x_discrete(drop=FALSE) + scale_fill_discrete(drop=FALSE) +
          labs(x="", y=HTML("densiteit (ind/m2)")) + scale_y_log10()
        # + facet_wrap(~jaar, ncol = 1, drop = FALSE)
      }
      else
      {
        ggplot(grafdat, aes(x=waterloop,y=biomassa+min(grafdat$biomassa[grafdat$biomassa!=0]), fill=fysiotoop)) +
          geom_boxplot() + scale_x_discrete(drop=FALSE) + scale_fill_discrete(drop=FALSE) +
          labs(x="", y=HTML("biomassa (g/m2)")) + scale_y_log10()
        # + facet_wrap(~jaar, ncol = 1, drop = FALSE)
      }
    }
  })
